...
class PVSystems(Base):
    ...
    def pvsystems_count(self):
        ...
        result = int(self.dss_obj.PVsystemsI(ctypes.c_int32(0), ctypes.c_int32(0)))
        return result

    def pvsystems_first(self):
        """Sets the first PVSystem to be active; returns 0 if none."""
        result = int(self.dss_obj.PVsystemsI(ctypes.c_int32(1), ctypes.c_int32(0)))
        return result

    def pvsystems_next(self):
        """Sets the next PVSystem to be active; returns 0 if none."""
        result = int(self.dss_obj.PVsystemsI(ctypes.c_int32(2), ctypes.c_int32(0)))
        return result

    def pvsystems_read_idx(self):
        """Gets the active PVSystem by index; 1..Count."""
        result = int(self.dss_obj.PVsystemsI(ctypes.c_int32(3), ctypes.c_int32(0)))
        return result

    def pvsystems_write_idx(self, argument):
        """Sets the active PVSystem by index; 1..Count.."""
        result = int(self.dss_obj.PVsystemsI(ctypes.c_int32(4), ctypes.c_int32(argument)))
        return result
