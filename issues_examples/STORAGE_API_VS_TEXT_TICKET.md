### Storage: API writes for `kvar`, `kw`, `amp_limit`, `amp_limit_gain` do not match `dss.text('? …')` or saved circuit
...
**Problem:** After setting `kvar`, `kw`, `amp_limit`, or `amp_limit_gain` via the py-dss-interface / DLL API, the Python getters show the new values, but `dss.text("? Storage.St.…")` still returns the original values. **`save circuit`** matches the `?` output, not those API getters.
