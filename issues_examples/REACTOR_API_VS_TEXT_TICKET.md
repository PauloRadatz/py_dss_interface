### Reactor: API writes for `kv`, `kvar`, `rp`, `l_curve`, `r_curve` do not match `dss.text('? …')` or saved circuit
...
**Problem:** After setting `kv`, `kvar`, `rp`, `l_curve`, or `r_curve` via the py-dss-interface / DLL API, the Python getters show the new values, but `dss.text("? Reactor.r.…")` still returns the original values. **`save circuit`** matches the `?` output, not those API getters.
