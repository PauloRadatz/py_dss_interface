### EnergyMeter: API writes for `metered_terminal`, `metered_element` do not match `dss.text('? …')` or saved circuit
...
**Problem:** After setting `metered_terminal` or `metered_element` via the py-dss-interface / DLL API, the Python getters show the new values, but `dss.text("? EnergyMeter.EM.…")` still returns the original values. **`save circuit`** matches the `?` output, not those API getters.
