### ISource: API writes for `amps`, `angle_deg`, `frequency` do not match `dss.text('? …')` or saved circuit
...
**Problem:** After setting `amps`, `angle_deg`, or `frequency` via the py-dss-interface / DLL API, the Python getters show the new values, but `dss.text("? ISource.ISrc.…")` still returns the original values. **`save circuit`** matches the `?` output, not those API getters.
