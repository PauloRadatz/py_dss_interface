### Vsource: API writes for `phases`, `base_kv`, `pu`, `frequency` do not match `dss.text('? …')` or saved circuit
...
**Problem:** After setting `phases`, `base_kv`, `pu`, or `frequency` via the py-dss-interface / DLL API, the Python getters show the new values, but `dss.text("? Vsource.V1.…")` still returns the original values. **`save circuit`** matches the `?` output, not those API getters.
